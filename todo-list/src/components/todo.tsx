    ...
    <div className="flex flex-col gap-2">
      {todos.map((item, idx) => {
        return (
          ...
          >
            <input checked = {item.isDone} type="checkbox" onChange={() => handleDone(item.id)} />
            <div className="text-2xl font-medium">{item.desc}</div>
            <button title={`${item.id}`} name={`${item.id}`}
              onClick={() => handleDelete(item.id)}
              className="w-8 h-8 border border-red-500 flex justify-center items-center rounded-md"
            >
              <svg
